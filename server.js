...
app.post("/api/pay-request", express.json({ limit: "10mb" }), async (req, res) => {
  try {
    ...
    const origPrice = batch?.price ? `₹${batch.price}` : "N/A";

    let priceLine = `💰 Amount: <b>${esc(origPrice)}</b>`;
    if (couponCode && discountPct && finalAmount != null) {
      priceLine =
        `💰 Original: <b>${esc(origPrice)}</b>\n` +
        `🎟 Coupon: <code>${esc(couponCode)}</code> (${esc(String(discountPct))}% off)\n` +
        `✅ Final Amount: <b>₹${esc(String(finalAmount))}</b>`;
    }
